"""Regenerate estimate-v1 summary and figure from verified metrics bundles."""
import json
...
from rz1t.estimate import ARMS, SEEDS, analyze_estimate, write_manifest

ROOT = Path(__file__).resolve().parents[1]
WORKS = (ROOT / "artifacts/estimate",)
OUT = ROOT / "reference/estimate-review"
...
def main():
    ...
    report = analyze_estimate(records, manifest)
    ...
    for arm in ARMS:
        ...
        ax.hlines(sum(nlls) / len(nlls), xs[arm] + offsets[arm] - 800, xs[arm] + offsets[arm] + 800,
                  color=colors[arm])
    ax.set(xlabel="Unique trainable body parameters", ylabel="Calibration NLL (nats/token)",
           title="Estimate-v1: unused calibration holdout, matched tokens, n=3")
    ax.grid(alpha=0.3)
    ax.legend()
    fig.tight_layout()
