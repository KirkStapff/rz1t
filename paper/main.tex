...
Checksummed metrics live in \path{artifacts/claim-a/}. Figures regenerate with \path{uv run python scripts/analyze\_claim\_a.py} and \path{uv run python scripts/make\_paper\_figures.py}. Full-state checkpoints are not in git. First-wave launcher provenance files inherit the label \path{duration-v1} from an earlier launcher; run configs and identities are claim-A. One registered U12 seed-$12$ run was retrained under the same identity after an infrastructure archive failure; it is not a replacement seed. Some earlier development checkpoints were not fully archived.
